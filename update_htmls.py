...
def update_file(filepath, injection_html):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # We want to replace everything from <div class="section">... all the way to </body>
    # Wait, the body has <!--BODY--> in some files.
    # Let's find the first `<div class="section">` after `<header>`
    start_tag = '<div class="section">'
    end_tag = '</body>'
    ...
    if start_idx != -1 and end_idx != -1:
        new_content = content[:start_idx] + injection_html + "\n" + content[end_idx:]
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Updated {filepath}")
    else:
        print(f"Failed to find tags in {filepath}")
...
buckles_opts = [f"Option {i}" for i in range(1, 21)]
broach_opts = [f"Option {i}" for i in range(1, 26)]
# format arrays exactly like JS literals

buckles_html = f"""<div class="section">
    <h3>Buckles & Broaches</h3>
    <div class="grid">
        <div class="card" onclick='openModal("Buckles", "images/placeholder.jpg", {str(buckles_opts).replace("'", '"')}, "Select Option")'>
            <img src="images/placeholder.jpg" alt="Buckles">
            <strong>Buckles</strong>
        </div>
        <div class="card" onclick='openModal("Broaches", "images/placeholder.jpg", {str(broach_opts).replace("'", '"')}, "Select Option")'>
...
mac_access_html = """<div class="section">
...
mac_parts_html = """<div class="section">
    <h3>Machine Parts</h3>
    <div class="grid">
...
base_dir = r"c:/Users/umara/OneDrive/Documents/codePulse websites/FAWZ23APR"
files = [
    ("beadsAndKits.html", beads_html),
    ("BoningBraCupsShoulderPads.html", boning_html),
    ("buckles&Broach.html", buckles_html),
    ("machineAccessories.html", mac_access_html),
    ("machineParts.html", mac_parts_html)
]

for filename, content in files:
    update_file(os.path.join(base_dir, filename), content)
